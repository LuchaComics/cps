...
import { faShoppingCart, faTasks, faBookOpen, faTachometer, faPlus, faDownload, faArrowLeft, faCheckCircle, faCheck, faGauge, faUsers, faEye } from '@fortawesome/free-solid-svg-icons'
...
function RetailerComicSubmissionAddStep3() {
    ...
    function onComicSubmissionDetailSuccess(response){
        ...
        // success page.
        if (response.creditId !== undefined && response.creditId !== null && response.creditId !== "" && response.creditId !== "000000000000000000000000") {
            console.log("onComicSubmissionDetailSuccess: user already paid, redirecting user to success page now");
        ...
        if (response.paymentProcessorPurchaseId !== undefined && response.paymentProcessorPurchaseId !== null && response.paymentProcessorPurchaseId !== "" && response.paymentProcessorPurchaseId.length > 0) {
            console.log("onComicSubmissionDetailSuccess: user already paid, redirecting user to success page now");
            setForceURL("/submissions/comics/add/"+response.id+"/confirmation")
            return
        }
        ...
        console.log("onComicSubmissionDetailSuccess: user must purchase", response.serviceType);
        ...
        <>
            <div class="container">
                <section class="section">
                    ...
                    {/* Page */}
                    <nav class="box">
                        <p class="title is-4"><FontAwesomeIcon className="fas" icon={faPlus} />&nbsp;New Online Comic Submission (Checkout)</p>
                        ...
                        {isFetching
                            ...
                            <>
                                <div class="container">
                                    <article>
                                        <p class="pb-3">
                                        You are now ready to checkout and purchase this comic submission. Please review before proceeding:
                                        </p>

                                        {offer !== undefined && offer !== null && offer !== "" && <article class="message pb-3" style={{width:"300px"}}>
                                            <div class="message-body">
                                                <p class="pb-1"><b>{offer.name}</b></p>
                                                <p class="pb-1">Comic: {submission.seriesTitle}</p>
                                                <p class="pb-1">Price: ${offer.price}&nbsp;{offer.priceCurrency}</p>
                                            </div>
                                        </article>}
                                    </article>
                                    ...
                                    <div class="columns pt-5">
                                        <div class="column is-half">
                                            {customerName === null
                                                ?
                                                <div class="">
                                                    <Link to={`/submissions/comics`} class="button is-medium is-secondary is-fullwidth-mobile">
                                                        <FontAwesomeIcon className="fas" icon={faArrowLeft} />&nbsp;Back to Comic Submissions
                                                    </Link>
                                                </div>
                                                :
                                                <div class="">
                                                    <Link to={`/customer/${customerID}/comics`} class="button is-medium is-secondary is-fullwidth-mobile">
                                                        <FontAwesomeIcon className="fas" icon={faArrowLeft} />&nbsp;Back to Customer
                                                    </Link>
                                                </div>
                                            }
                                        </div>
                                        <div class="column is-half has-text-right">
                                            <button onClick={(e)=>{onClick(id)}} class="button is-medium is-primary is-fullwidth-mobile" type="button">
                                                <FontAwesomeIcon className="fas" icon={faShoppingCart} />&nbsp;Checkout
                                            </button>
                                        </div>
                                    </div>
                                </div>
