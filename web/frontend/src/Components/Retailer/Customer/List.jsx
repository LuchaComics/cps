...
import { faFilter, faFilterCircleXmark, faArrowLeft, faUsers, faTachometer, faEye, faPencil, faTrashCan, faPlus, faGauge, faArrowRight, faTable, faRefresh, faSearch} from '@fortawesome/free-solid-svg-icons';
...
function RetailerCustomerList() {
    ...
    const [temporarySearchText, setTemporarySearchText] = useRecoilState(customerFilterTemporarySearchTextState); // Searching - The search field value as your writes their query.
    const [actualSearchText, setActualSearchText] = useRecoilState(customerFilterActualSearchTextState);          // Searching - The actual search query value to submit to the API.
    const [status, setStatus] = useRecoilState(customerFilterStatusState);                                        // Filtering
    const [createdAtGTE, setCreatedAtGTE] = useRecoilState(customerFilterJoinedAfterState);                       // Filtering
                    ...
                    <nav class="box">
                        <div class="columns">
                            <div class="column is-8">
                                <h1 class="title is-4">
                                    <FontAwesomeIcon className="fas" icon={faUsers} />&nbsp;Customer List for: {currentUser && currentUser.storeName}
                                </h1>
                            </div>

                            {/* Mobile only */}
                            <div className="column has-text-right is-hidden-desktop is-hidden-tablet">
                                <button onClick={()=>fetchList(currentCursor, pageSize, actualSearchText, status, createdAtGTE)} class="is-fullwidth button is-small is-info" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faRefresh} />&nbsp;<span class="is-hidden-desktop is-hidden-tablet">&nbsp;Refresh</span>
                                </button>
                                &nbsp;
                                <button onClick={(e)=>setShowFilter(!showFilter)} class="is-fullwidth button is-small is-success" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faFilter} />&nbsp;Filter
                                </button>
                                &nbsp;
                                <Link to={`/customers/add`} className="is-fullwidth button is-small is-primary" type="button">
                            ...
                            <div className="column has-text-right is-hidden-mobile">
                                ...
                                &nbsp;
                                <Link to={`/customers/add`} className="button is-small is-primary" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faPlus} />&nbsp;New
                                </Link>
                            </div>
                                ...
                                {customers && customers.results && (customers.results.length > 0 || previousCursors.length > 0)
                                    ...
                                    <section class="hero is-medium has-background-white-ter">
                                          <div class="hero-body">
                                            ...
                                            <p class="subtitle">
                                                No customers. <b><Link to="/customers/add">Click here&nbsp;<FontAwesomeIcon className="mdi" icon={faArrowRight} /></Link></b> to add your first customer.
                                            </p>
                                          </div>
                                    </section>
                                }
