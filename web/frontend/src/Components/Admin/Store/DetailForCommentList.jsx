...
import { getStoreDetailAPI, postStoreCreateCommentOperationAPI } from "../../../API/store";
...
function AdminStoreDetailForCommentList() {
    ...
    const onSubmitClick = (e) => {
        ...
        setErrors(null);
        postStoreCreateCommentOperationAPI(id, content, onStoreUpdateSuccess, onStoreUpdateError, onStoreUpdateDone, onUnauthorized);
    }
            ...
            <div class={`modal ${showStoreEditOptions ? 'is-active' : ''}`}>
                ...
                <div class="modal-card">
                    <header class="modal-card-head">
                        ...
                        <button class="delete" aria-label="close" onClick={(e)=>setShowStoreEditOptions(false)}></button>
                    </header>
                    <section class="modal-card-body">
                        ...
                        <br /><br />
                        <Link to={`/admin/store/${store.id}/store/search`} class="button is-primary">Pick a Different Store</Link>
                    </section>
                    <footer class="modal-card-foot">
                        <button class="button" onClick={(e)=>setShowStoreEditOptions(false)}>Close</button>
                    </footer>
                </div>
            </div>
                    ...
                    <nav class="box">
                        <div class="columns">
                            ...
                            <div class="is-hidden column has-text-right">
                                {/* Mobile Specific */}
                                <Link to={`/admin/submissions/comics/add?store_id=${id}&store_name=${store.name}`} class="button is-small is-success is-fullwidth is-hidden-desktop" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faPlus} />&nbsp;CPS
                                </Link>
                                {/* Desktop Specific */}
                                <Link to={`/admin/submissions/comics/add?store_id=${id}&store_name=${store.name}`} class="button is-small is-success is-hidden-touch" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faPlus} />&nbsp;CPS
                                </Link>
                            </div>
                        </div>
                                ...
                                {store && <div class="container">
                                    ...
                                    <div class="columns pt-4">
                                        <div class="column is-half">
                                            <Link to={`/admin/stores`} class="button is-fullwidth-mobile"><FontAwesomeIcon className="fas" icon={faArrowLeft} />&nbsp;Back to Stores</Link>
                                        </div>
                                        <div class="column is-half has-text-right">
                                            <button onClick={onSubmitClick} class="button is-primary is-fullwidth-mobile"><FontAwesomeIcon className="fas" icon={faPlus} />&nbsp;Add Comment</button>
                                        </div>
                                    </div>
