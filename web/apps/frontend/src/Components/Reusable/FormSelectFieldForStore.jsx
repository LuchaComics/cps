                ...
                <span class="select">
                   <select class={`input ${errorText && 'is-danger'} ${validationText && 'is-success'} has-text-black`}
                     ...
                     placeholder={`Pick the workout program type`}
                        onChange={(e)=>setStoreIDAndStoreName(e.target.value, e.target.options[e.target.selectedIndex].text)}
                        disabled={disabled}>
                       {workoutProgramTypeSelectOptions && workoutProgramTypeSelectOptions.length > 0 && workoutProgramTypeSelectOptions.map(function(option, i){
                           ...
                           // console.log("");
                           return <option selected={storeID === option.value} value={option.value}>{option.label}</option>;
                       })}
                   </select>
               </span>
            </div>
