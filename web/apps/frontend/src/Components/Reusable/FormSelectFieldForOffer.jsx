                ...
                <span class="select">
                   <select class={`input ${errorText && 'is-danger'} ${validationText && 'is-success'} has-text-black`}
                     ...
                     placeholder={`Pick the workout program type`}
                        ...
                        disabled={disabled}>
                       {offerSelectOptions && offerSelectOptions.length > 0 && offerSelectOptions.map(function(option, i){
                           // console.log("offerID", offerID);
                           // console.log("option.value", option.value);
                           // console.log(offerID, "===", option.value, "->>>", offerID === option.value);
                           // console.log("");
                           return <option selected={offerID === option.value} value={option.value}>{option.label}</option>;
                       })}
                   </select>
               </span>
            </div>
