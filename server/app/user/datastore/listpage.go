...
type UserPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
func (impl UserStorerImpl) newPaginationFilter(f *UserPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `name`, `lexical_name`, `created_at` and `modified_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl UserStorerImpl) newPaginationFilterBasedOnString(f *UserPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl UserStorerImpl) newPaginationFilterBasedOnTime(f *UserPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl UserStorerImpl) newPaginationOptions(f *UserPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl UserStorerImpl) newPaginatorNextCursor(f *UserPaginationListFilter, results []*User) (string, error) {
	var lastDatum *User
	...
	case "modified_at":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported fields are `name`, `created_at` and `modified_at`", f.SortField)
	}
