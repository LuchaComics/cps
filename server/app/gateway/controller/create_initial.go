...
import (
	...
	org_d "github.com/LuchaComics/cps-backend/app/store/datastore"
...
func (c *GatewayControllerImpl) createInitialRootAdmin(ctx context.Context) error {
	...
	if doesExist == false {
		...
		org := &org_d.Store{
			...
			Type:               org_d.RootType,
			Status:             org_d.StoreActiveStatus,
			Name:               c.Config.AppServer.InitialAdminStoreName,
			CreatedAt:          time.Now(),
			CreatedByUserName:  usr.Name,
			CreatedByUserID:    usr.ID,
			Comments:           []*org_d.StoreComment{},
			Timezone:           "America/Toronto",
		}
		err = c.StoreStorer.Create(ctx, org)
		if err != nil {
			c.Logger.Error("database create error", slog.Any("error", err))
			return err
		}
		...
		// Attach the user.
		usr.StoreID = org.ID
		usr.StoreName = org.Name
		usr.StoreTimezone = org.Timezone
